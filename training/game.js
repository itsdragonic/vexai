...
var World = function() {
    ...
    for (var k = 0; k < 30; k++) {
        ...
        var t = convnetjs.randi(1, 3);
        // food or poison (1 and 2)
...
World.prototype = {
    ...
    tick: function() {
            ...
            for (var j = 0, m = this.agents.length; j < m; j++) {
                ...
                if (d < it.rad + a.rad) {
                    ...
                    if (!rescheck) {
                        // ding! nom nom nom
                        if (it.type === 1)
                            a.digestion_signal += 5.0;
                        // mmm delicious apple
                        if (it.type === 2)
                            a.digestion_signal += -6.0;
                        // ewww poison
                        it.cleanup_ = true;
                        update_items = true;
                        break;
        ...
        if (this.items.length < 30 && this.clock % 10 === 0 && convnetjs.randf(0, 1) < 0.25) {
            ...
            var newitt = convnetjs.randi(1, 3);
            // food or poison (1 and 2)
...
var Agent = function() {
    ...
    // outputs on world
    this.rot1 = 0.0;
    // rotation speed of 1st wheel
    this.rot2 = 0.0;
    // rotation speed of 2nd wheel
...
function reload() {
    w.agents = [new Agent()];
    // this should simply work. I think... ;\
